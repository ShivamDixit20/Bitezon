...
exports.getPromotedRestaurants = async (req, res) => {
  try {
    const restaurants = await Restaurant.find({ platform: PLATFORM, promoted: true }).lean();
    res.json({ success: true, count: restaurants.length, data: restaurants.map(formatRestaurant) });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error fetching promoted restaurants', error: error.message });
  }
};
